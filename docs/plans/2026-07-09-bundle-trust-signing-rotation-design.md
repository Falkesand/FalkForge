...
> **Stage 2 landed (2026-07-09, `fix/bundle-payload-trust`).** Require-signed on the update path (B2), the executed-payload coverage extension, the key-epoch/revocation store, and the `forge extract`/`forge migrate` gates are implemented. B2 is closed on the update path and both extraction bypasses now bind byte→TOC→signed. The remaining C14 close-out (docs/provenance.md + the manual C14 section, and marking the assessment item DONE) happens after the Merge Gate confirms B1+B2 closed. Commits: 2a require-signed update path, 2b coverage extension, 2c epoch/revocation store, 2d extraction bypass gates. Stage 2 deviations/decisions, each deliberate:
> - **2a — require-signed via the launcher, not a bundle field.** The already-installed, already-trusted engine asserts `--require-signed` when it relaunches a downloaded update bundle (`DefaultUpdateLauncher.BuildRelaunchArguments`, alongside the existing `--base-bundle`). `Program.cs` parses it and threads it into the bootstrap/self-extract gate (`RunAsBootstrapper` + `VerifySignedPayloadTrust` → `SignedPayloadTocVerifier.Verify(..., requireSigned)`). So the trust decision is made by the trusted engine, not the downloaded artifact (design §3.4's "launcher-asserts" approach). A fresh install never receives the flag → unsigned bundles still run (allowed-but-warned asymmetry preserved). The unsigned fresh-install *warning* remains unemitted (Stage-1 carry-over, no logger at the gate).
> - **2b — coverage as a gate tightening, not a signer change.** The reference `BundleCompiler` and `DeltaBundleCompiler` already feed ONE payload list to both signing and embedding, so every embedded/executed payload is already signed — the residual hole was purely that the gates *skipped* unmatched TOC entries. `SignedPayloadTocVerifier` now REJECTS any TOC payload a signed bundle does not cover (INT004), turning "skip if unmatched" into "reject if unmatched." No signer change was needed (§5.4's "enumerate all TOC-producing payloads" is already satisfied by construction). The old `PayloadNotInSignedSet_…Accepts` unit test was flipped to assert rejection.
> - **2c — epoch/revocation store + the signed-bytes compat trap, resolved.** The envelope gains optional `epoch` (int) and `revoked` (string[]) fields. `ComputeSignedBytes(files, epoch, revoked)` folds them into the signed message **only when present** (non-zero epoch or non-empty revoked); with neutral values it returns the exact legacy files-only bytes. **Proven both directions by tests:** (i) neutral bytes are byte-identical to the legacy `ComputeSignedBytes(files)`, so v1 and Stage-1 v2 (epoch-0) envelopes still verify; (ii) tampering the epoch or the revocation list breaks the signature (INT001). Persisted store: `TrustState`/`TrustStateStore`/`TrustStateJsonContext` at `%ProgramData%\FalkForge\Trust\trust-state.json`, load-tolerant (missing = epoch 0), monotonic `Advance` (never lowers epoch; unions revocations) called ONLY after a verified apply (the engine bootstrapper advances it after a `Completed` update). Enforcement: `SignedPayloadTocVerifier.Verify` gained `storedEpoch` + `revokedFingerprints`; epoch below stored → **INT008**, accepted-signature fingerprint locally revoked → **INT001**. `IntegrityEnvelopeCodec.MatchTrustedSignature` returns the accepted fingerprint so the gate can enforce revocation. Builder/config: `IntegrityBuilder.Epoch/Revoke`, `IntegrityConfiguration.Epoch/RevokedFingerprints`, signer folds them in.
>   - **Deliberate strengthening beyond §6.3:** the design signs only the epoch; this ALSO binds the `revoked` list into the signed bytes. Rationale — with the epoch signed but revocations unsigned, an attacker could take a legit revoking-update, keep its epoch but strip its `revoked` list, so the client advances the epoch while recording no revocation and a same-epoch bundle signed by the revoked key would then be accepted. Signing `revoked` closes that; compat is preserved because an empty revocation list keeps the files-only bytes. Separator is U+001F (cannot occur in the files JSON, epoch digits, or hex fingerprints). OTA delivery of *new* trusted keys remains deferred (§10); only the local store + epoch check + the revoke-half data model shipped.
> - **2d — one shared verify helper, hoisted.** `BundleTrustVerifier.VerifyBundleContent` (new, `Engine.Protocol.Integrity`) + `BundleTrustJsonContext` (AOT-safe, mirrors `LayoutJsonContext`) deserialize the embedded manifest and call `SignedPayloadTocVerifier`. `forge extract`, `forge migrate`, AND the engine's own `VerifySignedPayloadTrust` all delegate to it (dedup, as §9 suggested). The CLI/decompiler pass an empty trusted set → inspection-grade: still rejects a post-signing overlay tamper (INT006) or an uncovered appended payload (INT004), but does not establish authorship. Proven: `forge extract` on a tampered signed bundle fails loud and writes no file (with a clean-bundle positive control).

> **Stage 3 landed (2026-07-09, `fix/bundle-payload-trust`).** Two independent reviews found the critical *not* actually closed after Stage 2: the require-signed flag was asserted TO the downloaded artifact rather than enforced by the trusted engine, and three defeaters remained. Stage 3 closes all four; each is its own green commit. C14 stays **open** pending the re-run adversarial gate.
> - **FIX 1 (BLOCKING) — verify the downloaded update IN-PROCESS before launch.** Stage 2 (2a) relaunched the downloaded EXE and passed it `--require-signed`, delegating trust to the very artifact it was meant to constrain (the downloaded EXE carries its own embedded engine and can ignore the flag). Now the already-installed, already-trusted engine verifies the *staged* bundle in-process before it advertises `UpdateReady` or launches: new `StagedUpdateVerifier` (`Engine.Integrity`) runs `BundleReader.Extract` + `BundleTrustVerifier.VerifyBundleContent(requireSigned: true)` against the baked trusted set, consulting the persisted anti-downgrade/revocation store. `UpdateDownloader.StartAsync` gates ready/launch on it and discards the staged bundle on failure; `UpdateService.LaunchReadyUpdate` re-verifies at UI-requested launch time (defends an on-disk swap). The launcher's `--require-signed` relaunch is kept as belt-and-suspenders but is NOT the security boundary. Proven end-to-end by driving the real `UpdateDownloader` + real `StagedUpdateVerifier` over real bundles (valid launches; unsigned/untrusted/tampered rejected and discarded).
> - **FIX 2 (HIGH) — require-signed fails closed on an empty trusted set.** An engine built with no `FalkForgeTrustedKey` has an empty set, and `MatchTrustedSignature` then falls back to consistency-only "accept any self-verifying signature" — so with require-signed on, a default-build engine accepted an attacker's re-signed update (B1 fail-open). `SignedPayloadTocVerifier.Verify` now rejects a present signature with no trust anchor when `requireSigned` is set (**INT009**, fail closed). The empty-set consistency-only acceptance stays legal only off the require-signed path (fresh install / inspection-grade CLI), so fresh installs are unchanged.
> - **FIX 3 (HIGH) — injective revoked-list signed bytes.** `ComputeSignedBytes` joined revoked fingerprints with a plain comma, so `["FP1","FP2"]` and `["FP1,FP2"]` produced identical signed bytes — an attacker could restructure a legit-signed revocation list without breaking the signature, then `TrustStateStore.Advance` recorded a bogus merged entry while the real keys were silently un-revoked (or a legit key force-revoked = DoS). The revoked list is now serialized with the same source-generated JSON as `files` (`IntegrityEnvelopeJsonContext.IReadOnlyListString`), which quotes each element, so distinct lists always produce distinct signed bytes. The neutral-case hinge is preserved: epoch 0 + empty revoked still returns the exact legacy files-only bytes (v1 / Stage-1 v2 bundles keep verifying) — proven by the existing byte-identity compat test plus a new injectivity + restructure-fails-verify pair.
> - **FIX 4 (MEDIUM) — ACL-harden the anti-downgrade store, with an elevation follow-up flag.** `trust-state.json` under `%ProgramData%\FalkForge\Trust\` claimed per-machine tamper-resistance but was created with default (inherited) ACLs, so an unprivileged process — including one running as the *same* interactive user, which inherits FullControl from `%ProgramData%` — could roll back the epoch / clear revocations. `TrustStateStore` now creates the store directory with a restrictive, inheritance-severed DACL: SYSTEM + Administrators FullControl, Users read-only (`FileSystemAclExtensions.CreateDirectory`, guarded by `OperatingSystem.IsWindows()`; an existing directory's ACL is left untouched). **Elevation finding / follow-up:** the engine bootstrapper's app.manifest is `asInvoker` (non-elevated) and `Advance` is called from it after a verified apply, so under the restrictive ACL a non-elevated advance is *denied* — the store therefore advances only when the engine runs elevated, and a standard-user write failure is surfaced as a `Result` failure (fail-loud, never silently dropped). Moving the store write to the elevated companion (`FalkForge.Engine.Elevation`) so every advance is elevated is architecturally non-trivial and is left as a tracked follow-up; the ACL (the tamper-resistance half) ships now. `TrustState`'s doc-comment was corrected to describe the ACL reality and this limitation rather than the previously-aspirational "only advanced during an elevated update apply."
> - **Fold-ins.** A mixed-envelope codec test (one malformed/untrusted entry + one valid trusted entry → accepted, exercising the `continue`-on-bad-entry paths). The `forge extract --require-signed` gate (`Program.cs VerifySignedPayloadTrust`) now threads `storedEpoch`/`revokedFingerprints` for consistency with the bootstrapper path. TrustedKeys.targets MSBuild normalization is left as an out-of-band note (no in-suite harness for the RoslynCodeTaskFactory target).

> **Stage 3 status / known limitation (2026-07-09).** The two RCE bypasses that made C14 critical — B1 (re-sign with an untrusted key) and B2 (strip the signature) — are **closed**: the already-installed, already-trusted engine verifies a downloaded update in-process against its baked trusted set with require-signed on, and fails closed on an empty trust anchor (INT009). What is **NOT yet active** is the epoch/revocation store's anti-downgrade/replay protection. The store (`TrustState`/`TrustStateStore`) and its enforcement checks (INT008 epoch, local revocation) all ship, and the store directory is ACL-hardened (SYSTEM/Admins FullControl, Users read-only, inheritance severed). But the only caller that *advances* the store is the engine bootstrapper, which runs `asInvoker` (non-elevated); under the restrictive ACL that write is denied, so in a normal standard-user run the epoch never advances past 0 and no revocation is ever recorded — the INT008/revocation checks therefore have nothing to enforce against. **In this release the engine does not block a version downgrade or replay.** Activation is tracked as follow-up **C16**: move the store write into the elevated companion (`FalkForge.Engine.Elevation`) so every advance is written elevated, and add ACL validation on load. Two related LOW hardening items also land under C16 tracking: the INT009 empty-set fail-closed mirror on `PayloadIntegrityGate` and the pre-launch re-verify in `UpdateDownloader`'s auto-launch path (both implemented on `fix/bundle-payload-trust`).

> **C16 landed — store ACTIVE (2026-07-10, `feature/c16-activate-trust-store`).** The anti-downgrade/replay store now genuinely enforces. Changes:
> - **Elevated write.** `TrustState`/`TrustStateStore`/`TrustStateJsonContext` moved to `FalkForge.Engine.Protocol.Integrity` (public) so the elevated companion can share them. A new whitelisted elevated command `TrustStateAdvance` (`FalkForge.Engine.Elevation.Commands.TrustStateAdvanceCommand`, registered in `ElevatedHost`) re-validates the payload (`TrustAdvancePayload`, bounded binary codec over the HMAC/PID-verified pipe), re-hardens the store directory, and writes the store under the restrictive ACL. The engine sends it after a verified+completed apply via `ElevatedTrustAdvancer` → `TrustStoreAdvanceCoordinator`, invoked from `ApplyStep` while the elevated companion from the install is still live (no second UAC). Plumbed through `EngineSessionOptions.AdvanceTrustStoreOnVerifiedApply` → `InstallerPipelineBuilder.WithTrustStoreAdvanceOnVerifiedApply` → `PipelineContext`. The old non-elevated `Program.cs` advance is removed.
> - **Anti-squat / ACL validation.** `TrustStateStore.IsDirectoryAclConforming` (inheritance severed + no broad-principal write grant), `EnsureSecuredDirectory` (create-if-absent hardened, or **reset** a non-conforming/loose directory — closing the pre-create squat), and `LoadValidated` (fail closed on a non-conforming directory rather than trusting an attacker-writable store) wired into the require-signed read paths (`StagedUpdateVerifier.VerifyWithBakedTrust`, both `Program.cs` update paths).
>
> **C16 HIGH remediation — targeted-squat / owner / epoch DoS closed (2026-07-10, `feature/c16-activate-trust-store`, `3616cd0`).** An adversarial review found the ACL conformance check was a *blacklist* of four broad SIDs (Users/World/Authenticated/Interactive) that also never checked the OWNER. A standard user who pre-creates `%ProgramData%\FalkForge\Trust` becomes its owner (ProgramData grants Users create-subfolder → CREATOR OWNER) and could set a protected DACL granting write to their *own specific SID* + SYSTEM/Admins Full + Users read; conformance returned TRUE, so `LoadValidated` trusted the attacker-written `trust-state.json` (set a saturating epoch to block all future updates, or clear revocations), and the owner also held implicit WRITE_DAC. Remediation (all TDD):
> - **Writer whitelist, not blacklist.** `IsDirectoryAclConforming` now rejects any Allow ACE granting a write-class right (write data / append / write attrs / delete / delete-child / change-permissions / take-ownership) held by *any* principal other than SYSTEM (S-1-5-18) or BUILTIN\Administrators (S-1-5-32-544) — so a targeted own-SID write ACE fails. Inheritance-severed requirement retained.
> - **Owner validation.** The directory owner must be SYSTEM or Administrators (owner has implicit WRITE_DAC), read via `GetOwner(typeof(SecurityIdentifier))`.
> - **Reset seizes ownership + purges foreign writers.** `ResetToRestrictiveDaclWindows` now `SetOwner(Administrators)` and purges every explicit ACE before re-adding SYSTEM/Admins FullControl + Users read-only; `EnsureSecuredDirectory` resets whenever the dir is non-conforming by the stricter check (also re-hardens a freshly-created dir whose owner defaulted to the elevated user). Runs in the elevated companion, where ownership seizure is permitted.
> - **Serviceability + epoch cap.** `LoadValidated`'s fail-closed message now gives accurate administrator recovery guidance (the old "reset with an elevated install" was misleading — a normal install never resets and this non-elevated read path cannot self-heal; full elevated self-heal deferred, see XML doc). `Advance` enforces an absolute epoch cap (`MaxEpoch = 1_000_000`), refusing (fail loud, not clamp) any advance above it so a compromised engine cannot saturate the epoch to `int.MaxValue` and permanently lock out every lower-epoch legitimate update (INT008). Monotonic + capped.
> - **Testability.** Conformance and reset shaping are extracted into pure in-memory cores (`IsAclConforming`, `ApplyRestrictiveDacl`) unit-tested without a real directory or elevation (`TrustStoreAclLogicTests`); the targeted-SID and non-admin-owner rejections are also proven end-to-end (`TrustStateStoreTests`). Filesystem tests that require an ownership-seizing commit are gated on an elevation check.
> - **Elevated-unavailable honesty.** If a run does not elevate, the store simply does not advance that run and the coordinator emits a loud warning — no false claim of protection. Enforcement stays correct for the elevated path; a failed elevated write is surfaced as an error, never swallowed.
> - **Enforcement now truthfully tested.** `TrustStoreEnforcementTests` drives the real advance → load → verify loop: an advanced epoch-N store rejects a replayed lower-epoch bundle (INT008) and a revoked-key bundle (INT001), while a genuine forward update still applies.
> - **Doc-comments flipped** from "dormant in this release" to ACTIVE in `TrustState.cs`/`TrustStateStore.cs`. The user-facing `documentation.html` §22 + `provenance.md` dormancy wording is left for a later docs pass (still accurate as historical context until then).
...
- **Authenticode / paid code-signing certificate.** Separate, optional layer. It does not cover the overlay payloads (they are appended after the stub is signed, past the signature's coverage) and already has an update-launcher seam (`DefaultUpdateLauncher` + `UpdatePublisherThumbprint`). Not required by this design and not a substitute for it.
