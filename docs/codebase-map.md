...
- PayloadReconstructionDispatcher (`src/FalkForge.Engine/PayloadReconstructionDispatcher.cs`): shared extract-or-reconstruct routing for a TOC entry — full payloads stream via BundleReader.ExtractPayloadToFile, delta payloads reconstruct against a base bundle via Engine.Protocol's DeltaApplicator.ReconstructPayloadToFile, failing loud when no base bundle is available
...
- Bootstrap/: PreUIBootstrapOrchestrator, PreUIPrerequisiteDetector/Installer (+ interfaces IPreUIPrerequisiteDetector/Installer), PreUIBootstrapOutcome, PreUIBootstrapResult, PreUIResult, IProgressSink, IProgressSinkFactory, DefaultBootstrapAdapters, WindowsFileSystemProvider, ElevationProbe/IElevationProbe, Native/ — native pre-UI prerequisite bootstrap (RunAsBootstrapper). Missing prerequisites on an unelevated run report failure with display names instead of relaunching elevated (the prior self-relaunch used the shell's runas verb, which resolves through a same-user-rewritable registry key, and has been removed).
...
Compilation/: BundleCompiler, ManifestGenerator, ManifestJsonContext, PayloadEmbedder, PayloadEntry, BundleContent, TocEntry, ElevationCompanionLocator/Appender, UiLocator/UiAppender
...
**UI executable** -- a runnable bundle also carries `FalkForge.Ui.exe` as a trust-covered payload (reserved TOC id `UiPayload.PackageId`, SHA-256 declared in `InstallerManifest.EngineUiSha256`, inside the ECDSA envelope when `Integrity()` is on). Appended before signing, because the bootstrapper hands the launched UI the session pipe secret and the engine behind that pipe holds an elevated gateway on a companion-carrying bundle. No opt-out: a bundle without a companion still installs per-user, a bundle without a UI cannot run at all. Resolution: explicit `UiPath` -> `FALKFORGE_UI` env -> beside a declared engine location (`EngineStubPath`, then the raw `FALKFORGE_ENGINE_STUB` value, never `EngineStubLocator`'s computed result) -> probing beside the host app, then the repo publish output. The binary must be a framework-dependent single-file publish; an apphost with `FalkForge.Ui.dll` beside it is rejected. `SignedPayloadTocVerifier` binds `EngineUiSha256` into the signed set both directions (INT012), and `BundleValidator` rejects an authored payload using the reserved id (BDL036).
...
## Demos
`demo/` (66 numbered demo directories — numbers 01-65, with 15 shared by `15-bundle-signing` and `15-msix-basic` — plus `demo/json/` 7 JSON configs). Full annotated list is in `documentation.html` §19 (Demos) — the manual is the maintained source. Highlights: 01 hello-world, 06 product-suite (bundle), 09 advanced-msi, 11/12/13 custom UI, 47 powershell-actions, 51 ice-validation, 53 delta-updates, 54 forge-migrate, 56 verify-and-plan, 57 reproducible-sbom, 59-63 signing (integrity/rotation/SignServer/require-signed/hybrid-PQ), 64 AcmeSuite capstone, 65 custom-dialog.
