...
public static partial class FirewallValidator
{
    ...
    private static void ValidateRule(FirewallRuleModel rule, List<FirewallValidationError> errors)
    {
        ...
        if (string.IsNullOrWhiteSpace(rule.Port) && string.IsNullOrWhiteSpace(rule.Program))
            errors.Add(new FirewallValidationError("FWL002",
                $"Firewall rule '{rule.Id}' must specify either a Port or a Program."));

        if (!string.IsNullOrWhiteSpace(rule.Port)) ValidatePort(rule.Port, rule.Id, "port", errors);

        if (!string.IsNullOrWhiteSpace(rule.RemotePort)) ValidatePort(rule.RemotePort, rule.Id, "remote port", errors);
    }
    ...
    private static void ValidatePort(string port, string ruleId, string portLabel, List<FirewallValidationError> errors)
    {
        ...
        {
            errors.Add(new FirewallValidationError("FWL003",
                $"Firewall rule '{ruleId}' has invalid {portLabel} format '{port}'. Expected a number (e.g. '8080') or a range (e.g. '8080-8090')."));
            return;
        }
        ...
        if (dashIndex < 0)
        {
            ...
            if (portNumber < 1 || portNumber > 65535)
                errors.Add(new FirewallValidationError("FWL003",
                    $"Firewall rule '{ruleId}' has invalid {portLabel} '{port}'. Port must be between 1 and 65535."));
        }
        else
        {
            ...
            if (start < 1 || start > 65535 || end < 1 || end > 65535)
                errors.Add(new FirewallValidationError("FWL003",
                    $"Firewall rule '{ruleId}' has invalid {portLabel} range '{port}'. Ports must be between 1 and 65535."));
            else if (start > end)
                errors.Add(new FirewallValidationError("FWL003",
                    $"Firewall rule '{ruleId}' has invalid {portLabel} range '{port}'. Start port must not exceed end port."));
        }
    }
