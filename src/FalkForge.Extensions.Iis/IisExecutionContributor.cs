...
/// deferred, elevated custom actions creating pools + sites (with all bindings, SSL certificate binding, and
/// sub-applications) on install and removing them on uninstall (with rollback on a failed install). Mirrors
/// <c>SqlExecutionContributor</c> / <c>FirewallExecutionContributor</c>.
/// </summary>
internal sealed class IisExecutionContributor(
    Func<IReadOnlyList<AppPoolModel>> pools,
    Func<IReadOnlyList<WebSiteModel>> sites,
    Func<IReadOnlyList<CertificateModel>> certificates) : IExecutionContributor
{
    public IReadOnlyList<ExecutionStep> GetExecutionSteps(ExtensionContext context)
        => IisCommandFactory.BuildSteps(pools(), sites(), certificates());
}
