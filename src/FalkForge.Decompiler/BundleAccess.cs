                ...
                for (var i = 0; i <= bytesRead - Magic.Length; i++)
                {
                    if (buffer[i] == Magic[0])
                    {
                        ...
                        if (matches)
                        {
                            ...
                            // triggers a ~2 GiB heap allocation (DoS vector). Shared with
                            // BundleReader — both parse the identical FALKBUNDLE manifest region,
                            // so the cap is single-sourced from BundleReader.MaxManifestBytes.
                            if (manifestLength < 0 || manifestLength > BundleReader.MaxManifestBytes)
                                return Result<InstallerManifest>.Failure(ErrorKind.BundleError,
                                    $"Invalid manifest length: {manifestLength}. " +
                                    $"Manifest length must be between 0 and {BundleReader.MaxManifestBytes} bytes (BDC003).");
