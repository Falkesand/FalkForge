...
public sealed class MockRegistry : IRegistry
{
    ...
    // with no root prefix — TryReadSubKeyNames/TryGetStringValue/TryGetDWordValue/TryValueExists/
    // TryKeyExists compare against their `subKey` parameter directly, never against a root-qualified
    // string. Because the root is never part of the comparison, one prefix simulates a read failure under
    // LocalMachine and CurrentUser alike.
    private readonly List<string> _failReadPrefixes = [];

    /// <summary>
    /// Makes <see cref="TryReadSubKeyNames"/>, <see cref="TryGetStringValue"/>,
    /// <see cref="TryGetDWordValue"/>, <see cref="TryValueExists"/>, and <see cref="TryKeyExists"/> return
    /// a <c>Failure</c> for any subkey path under <paramref name="subKeyPrefix"/>. The match is against the
    ...
    public Result<int?> TryGetDWordValue(RegistryRoot rootKey, string subKey, string valueName)
    {
        ...
        return Result<int?>.Success(GetDWordValue(rootKey, subKey, valueName));
    }
