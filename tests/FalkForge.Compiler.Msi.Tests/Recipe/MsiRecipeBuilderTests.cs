...
public sealed class MsiRecipeBuilderTests
{
    ...
    {
        // Phase 4 wires in twenty built-in producers (Property, Directory,
        // Feature, Component, File, FeatureComponents, Condition, Upgrade,
        // Media, Registry, RemoveRegistry, ServiceInstall, ServiceControl,
        // Shortcut, Environment, Font, LaunchCondition, IniFile, MoveFile,
        // RemoveFile). With an empty resolved package each producer emits
        // zero rows but the table itself is still present so downstream
        // phases see a stable table set. The recipe's Tables array therefore
        // contains twenty tables, not zero.
        ...
        Assert.Equal(20, recipe.Tables.Length);
