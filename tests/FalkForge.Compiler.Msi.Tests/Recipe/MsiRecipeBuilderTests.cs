...
public sealed class MsiRecipeBuilderTests
{
    ...
    {
        // Phase 4 wires in eighteen built-in producers (Property, Directory,
        // Feature, Component, File, FeatureComponents, Condition, Upgrade,
        // Media, Registry, RemoveRegistry, ServiceInstall, ServiceControl,
        // Shortcut, Environment, Font, MoveFile, RemoveFile). With an empty
        // resolved package each producer emits zero rows but the table itself
        // is still present so downstream phases see a stable table set. The
        // recipe's Tables array therefore contains eighteen tables, not zero.
        ...
        Assert.Equal(18, recipe.Tables.Length);
