...
public sealed class MsiRecipeBuilderTests
{
    ...
    {
        // Phase 4 wires in seventeen built-in producers (Property, Directory,
        // Feature, Component, File, FeatureComponents, Condition, Upgrade,
        // Media, Registry, RemoveRegistry, ServiceInstall, ServiceControl,
        // Shortcut, Environment, MoveFile, RemoveFile). With an empty resolved
        // package each producer emits zero rows but the table itself is still
        // present so downstream phases see a stable table set. The recipe's
        // Tables array therefore contains seventeen tables, not zero.
        ...
        Assert.Equal(17, recipe.Tables.Length);
