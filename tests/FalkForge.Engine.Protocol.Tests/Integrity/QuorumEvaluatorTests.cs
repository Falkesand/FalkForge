...
public sealed class QuorumEvaluatorTests
{
    ...
    //
    // A prior version of this note also covered a `slotToSig` array (a per-slot mirror of
    // `sigToSlot`, sentinel-filled with `Array.Fill(slotToSig, -1)` and written on every match) that
    // was write-only — nothing in Evaluate or TryAssign ever read it back, so its sentinel mutating
    // from `-1` to `+1` was unobservable. That array has since been removed entirely rather than
    // left as dead state for future Stryker sweeps to keep flagging.
}
