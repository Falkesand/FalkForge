namespace FalkInstaller.Engine.Tests.Logging;
...
public sealed class EngineLoggerTests : IDisposable
{
    ...
    {
        _tempDir = Path.Combine(Path.GetTempPath(), "FalkInstaller_Tests", Guid.NewGuid().ToString("N"));
    ...
    [Fact]
    public void GetDefaultLogPath_ContainsFalkInstallerDirectory()
    {
        var path = EngineLogger.GetDefaultLogPath();
        Assert.Contains("FalkInstaller", path);
