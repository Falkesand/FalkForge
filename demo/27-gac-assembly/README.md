...
Registers a .NET assembly in the Global Assembly Cache (GAC), making it available to all .NET applications on the
machine.
...
- `FileRef()` references the file by the name used in `Files()`. The file is deployed to the specified directory and
  also registered in the GAC.
- GAC registration is primarily relevant for .NET Framework assemblies. .NET Core / .NET 5+ applications typically do
  not use the GAC.
