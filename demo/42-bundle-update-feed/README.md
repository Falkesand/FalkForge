...
Configures the bundle to check a remote JSON feed for available updates. The bootstrapper can notify the user about newer versions or automatically trigger an update, depending on the chosen policy.
...
- The feed URL should return a JSON document that the bootstrapper engine can parse to determine whether a newer version is available.
- `UpdatePolicy.NotifyOnly` informs the user without forcing an update. Other policies may allow silent or automatic updates.
