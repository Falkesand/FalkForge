...
Configures Windows Firewall rules as part of an MSI installer. This demo creates an inbound TCP rule that opens port
8080 for HTTP traffic, using the fluent `FirewallExtension` API with built-in validation.
...
- In production, extensions register automatically via the FalkForge SDK extension pipeline during compilation. This
  demo shows the MSI structure; extension tables are emitted by the SDK at build time.
